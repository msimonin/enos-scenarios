...
Vagrant.configure("2") do |config|
 ...
 # Share a directory between the host and guest OS 
...
#+BEGIN_COMMENT
- Adrien: 
 + I didn't succeed to put ~openstack_release: "4.0.2"~
 + Moreove, I would rather prefer to have a kind of WARNING box to highlight this area in a different manner than the usual information block
#+END_COMMENT

#+BEGIN_NOTE
*Please note that with  the box you downloaded, it is mandatory to specify the openstack version in the configuration file.*
Please edit the reservation.yaml and add the line ~openstack_release:...~"  in the ~global.yml~ section:
#+BEGIN_EXAMPLE 
 # globals.yml
kolla:
