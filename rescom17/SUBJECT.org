...
~rally~ key specifies the list of ~scenarios~ to execute (here, only
Nova boot and list servers).

TODO: ...

A Rally scenario is parameterable.

the number of OpenStack clients run by Rally
(~concurrency~), the number of ~times~ a scenario is executed and  Rally scenarios
